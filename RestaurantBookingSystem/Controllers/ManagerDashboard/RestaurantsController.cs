...
namespace RestaurantBookingSystem.Controllers
{
    ...
    public class RestaurantsController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetRestaurantsByManagerId(int managerId)
        {
            var restaurants = await _service.GetByManagerIdAsync(managerId);
            return Ok(restaurants);
        }
        ...
        {
            var manager = await _service.GetManagerByUserIdAsync(userId);
            
            if (manager == null)
                return NotFound(new ApiErrorResponse
                {
                    Success = false,
                    Message = "Manager not found for this user"
                });

            return Ok(new ApiSuccessResponse<ManagerDto>
            {
