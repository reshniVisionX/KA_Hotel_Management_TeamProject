...
namespace RestaurantBookingSystem.Services
{
    public class RestaurantsService : IRestaurantService
    {
        ...
        public async Task<IEnumerable<RestaurantsDto>> GetByManagerIdAsync(int managerId)
        {
            var restaurants = await _repo.GetByManagerIdAsync(managerId);
            return restaurants.Select(MapToDto);
        }

        public async Task<ManagerDto?> GetManagerByUserIdAsync(int userId)
        {
            var manager = await _repo.GetManagerByUserIdAsync(userId);
            return manager == null ? null : MapToManagerDto(manager);
        }
        ...
        private static ManagerDto MapToManagerDto(RestaurantBookingSystem.Model.Manager.ManagerDetails m)
        {
