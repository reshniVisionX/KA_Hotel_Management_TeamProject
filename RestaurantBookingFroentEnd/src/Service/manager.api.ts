...
export async function getCustomersByRestaurant(restaurantId: number) {
  try {
    const res = await axios.get<Customer[] | ApiSuccessResponse<Customer[]> | ApiErrorResponse>(
...
export async function getAllRestaurants() {
  try {
    const res = await axios.get<Restaurant[] | ApiSuccessResponse<Restaurant[]> | ApiErrorResponse>(
      `${API_URL}/Restaurants`
    );
...
export async function getMenuItemsByRestaurant(restaurantId: number) {
  try {
    const res = await axios.get<MenuItem[] | ApiSuccessResponse<MenuItem[]> | ApiErrorResponse>(
...
// DineIn (Table Management) endpoints
...
// Orders endpoints (DineIn/DineOut)
...
// Review endpoints (Read-only for managers)
