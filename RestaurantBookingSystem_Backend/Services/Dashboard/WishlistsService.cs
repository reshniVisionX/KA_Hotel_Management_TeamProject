    ...
    public class WishlistsService : IWishlistsService
    {
        ...
        public async Task<List<WishlistsDto>> GetUserWishlistAsync(int userId)
        {
            ...
            return wishlists.Select(w => new WishlistsDto
            {
                ...
                RestaurantImage = w.Restaurant?.Images,
                RestaurantCategory = w.Restaurant?.RestaurantCategory.ToString(),
                RestaurantRating = w.Restaurant?.Ratings,
                Location = w.Restaurant?.Location,

                // Menu Item Details
                ItemName = w.Item?.ItemName,
                ItemDescription = w.Item?.Description,
                ItemPrice = w.Item?.Price,
                ItemImage = w.Item?.Image,
                IsVegetarian = w.Item?.IsVegetarian
            }).ToList();
        }
