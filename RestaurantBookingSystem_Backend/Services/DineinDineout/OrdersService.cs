    ...
    public class OrdersService
    {
        ...
        public async Task<OrderSummaryDto> GetOrderSummaryAsync(int userId)
        {
            ...
            if (pendingOrder == null)
                throw new KeyNotFoundException($"No pending order found for user {userId}");
        ...
        public async Task<IEnumerable<Orders>> GetByRestaurantAsync(int restaurantId)
        {
            ...
            if (!orders.Any())
                throw new KeyNotFoundException($"No orders found for restaurant {restaurantId}");
        ...
        {
            // Basic validation
            if (dto.RestaurantId <= 0) throw new ArgumentException("Invalid RestaurantId");
            ...
            if (!cartItems.Any())
                throw new InvalidOperationException($"No items found in cart for user {dto.UserId} and restaurant {dto.RestaurantId}");
            ...
            foreach (var cartItem in cartItems)
            {
                ...
                if (menuItem.RestaurantId != dto.RestaurantId)
                    throw new InvalidOperationException($"Item {cartItem.ItemId} does not belong to restaurant {dto.RestaurantId}");
        ...
        public async Task<OrderPreviewDto> GetOrderPreviewAsync(int userId, int restaurantId)
        {
            ...
            if (!cartItems.Any())
                throw new InvalidOperationException($"No items found in cart for user {userId} and restaurant {restaurantId}");
            ...
            foreach (var cartItem in cartItems)
            {
                ...
                if (menuItem.RestaurantId != restaurantId)
                    throw new InvalidOperationException($"Item {cartItem.ItemId} does not belong to restaurant {restaurantId}");
        ...
        public async Task<Orders> CreateFromCartAsync(int userId)
        {
            ...
            if (!cartItems.Any())
                throw new InvalidOperationException($"No items found in cart for user {userId}");
            ...
            if (restaurantIds.Count > 1)
                throw new InvalidOperationException("All items in cart must be from the same restaurant");
